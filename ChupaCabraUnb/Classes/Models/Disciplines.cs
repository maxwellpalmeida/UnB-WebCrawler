...
namespace UnBWebCrawler.Classes.Models
{
    public class Disiplines : Common.PageContent
    {
        ...
        private async void LoadRequirements()
        {
            ...
            //this.Requirements = document.DocumentNode.Descendants("table").Where(w => w.GetAttributeValue("id", "").Equals("datatable"))?.FirstOrDefault()
            //    ?.Descendants("tr")?.ElementAt(5)?.Descendants("a")?.FirstOrDefault()?.InnerText ?? "";
        }
    }
